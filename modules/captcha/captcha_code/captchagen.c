/*
 * captchagen.c
 * Generates Captcha Images and puts in specific directory
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int main(int argc, char *argv[])
{
  ...
  fprintf(stderr, "lines=%d (first line contains number of words), nwords=%lu.\n", i, nwords);
  fclose(fp);
...
#define CN 10
  ...
  for (x = 0; x < nwords; x++) {
    ...
    sprintf(captcha_file, "%s/%d.%s", captcha_dir, x, captcha_file_fmt);
    ...
    fprintf(stderr, "Image written to file [%ld bytes]: %s\n", nwritten, captcha_file);
    close(fd);
    MagickRelinquishMemory(blob);
  }
